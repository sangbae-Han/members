...
export function exportMembersToExcel(members: Member[]) {
  ...
  ws1["!cols"] = [
    ...
    { wch: 8 }, { wch: 16 }, { wch: 12 }, { wch: 8 }, { wch: 24 }, { wch: 30 }, { wch: 30 },
  ];
  ...
  const ws3 = XLSX.utils.json_to_sheet(
    aidRows.length ? aidRows : [{ 이름: "", 차량번호: "", 일자: "", 구분: "", 상조내역: "", 지원금액: "" }]
  );
  ws3["!cols"] = [{ wch: 10 }, { wch: 16 }, { wch: 12 }, { wch: 10 }, { wch: 30 }, { wch: 12 }];
  XLSX.utils.book_append_sheet(wb, ws3, "상조현황");

  const filename = `전일련회원목록_${new Date().toISOString().slice(0, 10)}.xlsx`;
      ...
      try {
        ...
        const members: Member[] = rows.map((row, i) => {
          ...
          return {
            ...
            garage,
            phone,
            joinDate: String(row["가입일"] ?? "").trim(),
            vehicleType,
            specialEquipments,
            vehiclePhoto: "",
            roadAddress: String(row["도로명주소"] ?? "").trim(),
            mailingAddress: String(row["우편주소"] ?? "").trim(),
            disciplineRecords: [],
            mutualAidRecords: [],
          };
        });
