...
import { exportMembersToExcel, importMembersFromExcel } from "../components/excelUtils";
import { parseQuery, filterByParsedQuery, SUGGESTIONS } from "../components/smartSearch";
  ...
  const confirmImport = (mode: "replace" | "merge") => {
    if (importState) {
      ...
      const msg = mode === "replace"
        ? `${importState.members.length}명으로 교체 저장되었습니다.`
        : `${importState.members.length}명이 추가 저장되었습니다.`;
      alert(msg);
    }
  };
        ...
        <div className="bg-card rounded-xl border border-border p-4 flex flex-wrap items-center gap-3">
          ...
          <div className="relative flex-1 min-w-48">
            ...
            <input
              ...
              className="w-full bg-input-background border border-border rounded-lg pl-9 pr-10 py-2 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all"
            />
          ...
          <select value={filterStatus} onChange={e => { setFilterStatus(e.target.value); setPage(1); }}
            className="bg-input-background border border-border rounded-lg px-3 py-2 text-sm outline-none focus:border-primary">
