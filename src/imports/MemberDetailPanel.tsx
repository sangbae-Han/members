...
import { X, Edit2, Trash2, Phone, Truck, MapPin, Users, Tag, CalendarDays, AlertTriangle, HeartHandshake } from "lucide-react";
import type { Member, DisciplineRecord, MutualAidRecord } from "../components/types";
        ...
        <div className="flex gap-1 mt-3 bg-white/10 rounded-lg p-1">
          {tabs.map(t => (
            <button key={t.key} onClick={() => setTab(t.key)}
              ...
              {t.label}
              {t.badge ? (
                <span className={`absolute -top-1 -right-1 w-4 h-4 rounded-full text-white flex items-center justify-center text-xs ${t.key === "discipline" ? "bg-red-500" : "bg-blue-500"}`}
