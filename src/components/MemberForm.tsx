  ...
  return (
    <div className="flex flex-col h-full bg-background">
      ...
      >
                ...
                <label key={g} className="flex items-center gap-2 cursor-pointer">
                  <div
                    ...
                    className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors cursor-pointer ${
                      form.grade === g ? "border-primary bg-primary" : "border-border bg-white"
                    }`}
                  >
              ...
              {(["활성", "비활성"] as MemberStatus[]).map((s) => (
                <label key={s} className="flex items-center gap-2 cursor-pointer">
                  <div
                    ...
                    className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors cursor-pointer ${
                      form.status === s
                        ? s === "활성" ? "border-emerald-500 bg-emerald-500" : "border-red-500 bg-red-500"
            ...
            {VEHICLE_TYPES.map((vt) => (
              <label key={vt} className="flex items-center gap-2 cursor-pointer">
                <div
                  ...
                  className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors cursor-pointer ${
                    form.vehicleType === vt ? "border-primary bg-primary" : "border-border bg-white"
                  }`}
                >
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border transition-colors text-sm ${
                    checked ? "bg-primary/10 border-primary text-primary" : "bg-muted border-border text-muted-foreground"
                  }`}
                >
                  <div
        ...
        {/* 주소 */}
        <div className="bg-card rounded-2xl p-4 shadow-sm space-y-4">
          <p className="text-foreground font-medium" style={{ fontSize: "13px" }}>주소</p>
          <Field label="도로명주소">
            <input className={inputClass} value={form.roadAddress} onChange={set("roadAddress")} placeholder="도로명주소" />
          </Field>
          <Field label="우편주소">
            <input className={inputClass} value={form.mailingAddress} onChange={set("mailingAddress")} placeholder="우편주소" />
          </Field>
        </div>
