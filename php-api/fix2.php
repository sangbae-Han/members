...
// 새 api.php 내용 (snake_case 컬럼명 사용)
$newApi = <<<'PHPEOF'
<?php
require_once __DIR__ . '/config.php';
...
function initTables(): void {
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS jil_members (
        ...
        special_equipments TEXT                 DEFAULT '',
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS jil_discipline (
        ...
        discipline_detail TEXT                 DEFAULT '',
    ...
    // 기존 테이블 컬럼 추가
    $alters = [
        "ALTER TABLE jil_members ADD COLUMN postal_code VARCHAR(10) DEFAULT '' AFTER vehicle_photo",
        "ALTER TABLE jil_members ADD COLUMN address_detail TEXT DEFAULT '' AFTER road_address",
    ];
...
function getMembers(): array {
    ...
    foreach ($members as $m) {
        $id = $m['id'];
        $eq = [];
        $raw = $m['special_equipments'] ?? '';
        if (!empty($raw)) {
            $decoded = json_decode($raw, true);
            $eq = is_array($decoded) ? $decoded : array_filter(explode(',', $raw));
        }
        $disc = array_map(fn($r) => [
            'id'               => $r['id'],
            'date'             => $r['date'],
...
function saveMembers(array $members): void {
    ...
    $mSql = "INSERT INTO jil_members
        (id,name,vehicle_number,grade,status,garage,phone,join_date,vehicle_type,
         special_equipments,vehicle_photo,postal_code,road_address,address_detail,mailing_address)
        ...
        ON DUPLICATE KEY UPDATE
          ...
          special_equipments=VALUES(special_equipments),vehicle_photo=VALUES(vehicle_photo),
    ...
    try {
        foreach ($members as $m) {
            ...
            $mIds[] = $id;
            $mStmt->execute([
                ':id'=>$id,':name'=>$m['name']??'',':vehicle_number'=>$m['vehicleNumber']??'',
                ':grade'=>$m['grade']??'일반',':status'=>$m['status']??'활성',':garage'=>$m['garage']??'서울',
                ':phone'=>$m['phone']??'',':join_date'=>$m['joinDate']??'',':vehicle_type'=>$m['vehicleType']??'1톤',
                ':special_equipments'=>json_encode($m['specialEquipments']??[],JSON_UNESCAPED_UNICODE),
                ':vehicle_photo'=>$m['vehiclePhoto']??'',':postal_code'=>$m['postalCode']??'',
                ':road_address'=>$m['roadAddress']??'',':address_detail'=>$m['addressDetail']??'',
...
switch ($action) {
    case 'ping':
        ...
        out(['ok'=>true,'version'=>'7.0','time'=>date('c'),'tables'=>$tables]);
...
if (file_put_contents($dest, $newApi) === false) {
    echo "<pre style='color:red'>❌ api.php 저장 실패 (권한 오류)</pre>";
} else {
    echo "<pre style='font-family:monospace;padding:2rem;background:#1a1a2e;color:#69f0ae;font-size:1rem'>";
    echo "✅ api.php 교체 완료 (버전 7.0 - snake_case 컬럼 지원)\n\n";
    echo "확인 중...\n";
    // 즉시 테스트
    $pdo = getDB();
    $row = $pdo->query("SELECT id, vehicle_number, join_date FROM jil_members LIMIT 1")->fetch();
    echo "✅ jil_members 조회: " . json_encode($row, JSON_UNESCAPED_UNICODE) . "\n\n";
    echo "완료! 앱으로 이동하세요 → <a href='../index.html' style='color:#4fc3f7'>클릭</a>";
    echo "</pre>";
}
